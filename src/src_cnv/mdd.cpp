...
void Layer::popul_layer(int siz, int lvl, int mdd_count, bool last) {
	if (lvl == 0) {
		...
		nodes.back()->emplace_back(new Node(siz + 1, lvl, mdd_count));				//node with state = |S2| + 1	
		for (size_t i = 0; i < lvl - 1; ++i) {								//gap nodes
...
void MDD::create_arcs_r(vector<vector<Node*>*>& nodes1, vector<vector<Node*>*>& nodes2, int pos, size_t mdd_count) {
	...
	if (ext_gap_ref[0][1] + psa_mats[mdd_count]->back()[siz2 - nodes2[0]->back()->lvl].pes_val + 0.0001 >= bound) {
		...
		nodes1[0]->at(0)->gap_child = new Arc(ext_gap_ref[0][1], nodes2[0]->back(), nodes1[0]->at(0));			//opening gap arc (|S2| + 1 -> |S2| + 1)
		nodes2[0]->back()->gap_parent = nodes1[0]->at(0)->gap_child;
		nodes2[0]->back()->lng_arc = nodes1[0]->at(0)->gap_child;
	}
...
void MDD::create_arcs(vector<vector<Node*>*>& nodes1, vector<vector<Node*>*>& nodes2, int lvl, int pos, size_t mdd_count) {
	...
	if (nodes1[0]->back()->gap_parent == NULL) {
		nodes1[0]->back()->active = 0;
		//delete nodes1[0]->back();
		//nodes1[0]->pop_back();
	}
	else {
		bool active = 0;
		if (nodes1[0]->back()->max_to[0] + ext_gap_ref[lvl][1] + psa_mats[mdd_count]->back()[siz2 - nodes2[0]->back()->lvl].pes_val + 0.0001 >= bound) {
			nodes2[0]->back()->max_to[0] = nodes1[0]->back()->max_to[0] + ext_gap_ref[lvl][1];
			nodes1[0]->back()->gap_child = new Arc(ext_gap_ref[lvl][1], nodes2[0]->back(), nodes1[0]->back());		//extension arc for node state = |S2| + 1
			nodes2[0]->back()->gap_parent = nodes1[0]->back()->gap_child;
			nodes2[0]->back()->lng_arc = nodes1[0]->back()->gap_child;
			active = 1;
		}
		for (size_t j = 0; j < siz; ++j) {														//alignment arcs for node state = |S2| + 1
			if (nodes1[0]->back()->max_to[0] + value_ref[pos][j] + ext_gap_ref[lvl][siz - j - 1] + psa_mats[mdd_count]->at(nodes2[0]->at(j)->state - 1)[siz2 - nodes2[0]->at(j)->lvl].val + 0.0001 >= bound) {
				nodes2[0]->at(j)->max_to[0] = nodes1[0]->back()->max_to[0] + value_ref[pos][j] + ext_gap_ref[lvl][siz - j - 1];
				nodes1[0]->back()->children.emplace_back(new Arc(value_ref[pos][j] + ext_gap_ref[lvl][siz - j - 1], nodes2[0]->at(j), nodes1[0]->back()));
		...
		if (!active) {
			nodes1[0]->back()->gap_parent->reward[0] = -IloInfinity;
	...
	for (int k = lvl - 1; k >= 0; --k) {
		for (int i = siz - 1; i >= 0; --i) {													//defining alignment arcs for nodes state > 2
			if (nodes1[k]->at(i)->gap_parent == NULL && nodes1[k]->at(i)->parents.empty()) {
				nodes1[k]->at(i)->active = 0;
				//delete nodes1[k]->at(i);
				//nodes1[k]->erase(nodes1[k]->begin() + i);
			}
			else {
				bool active = 0;
				...
				if (nodes1[k]->at(i)->max_to[0] + ext_gap_ref[k][1] + psa_mats[mdd_count]->at(nodes2[k + 1]->at(i)->state - 1)[siz2 - nodes2[k + 1]->at(i)->lvl].pes_val + 0.0001 >= bound) {
					nodes2[k + 1]->at(i)->max_to[0] = nodes1[k]->at(i)->max_to[0] + ext_gap_ref[k][1];
					nodes1[k]->at(i)->gap_child = new Arc(ext_gap_ref[k][1], nodes2[k + 1]->at(i), nodes1[k]->at(i));		//defining gap arc for nodes state > 2
					nodes2[k + 1]->at(i)->gap_parent = nodes1[k]->at(i)->gap_child;
					nodes2[k + 1]->at(i)->lng_arc = nodes1[k]->at(i)->gap_child;
					active = 1;
				}
				if (!active) {
					if (nodes1[k]->at(i)->gap_parent != NULL)
						nodes1[k]->at(i)->gap_parent->reward[0] = -IloInfinity;
...
void MDD::create_arcs_t(vector<vector<Node*>*>& nodes1, vector<vector<Node*>*>& nodes2, int lvl, int pos, size_t mdd_count) {
	...
	if (nodes1[0]->back()->gap_parent == NULL) {
		...
		bool active = 0;
		if (nodes1[0]->back()->max_to[0] + ext_gap_ref[lvl][1 + siz] + 0.0001 >= bound) {
			nodes2[0]->back()->max_to[0] = nodes1[0]->back()->max_to[0] + ext_gap_ref[lvl][1 + siz];
			nodes1[0]->back()->gap_child = new Arc(ext_gap_ref[lvl][1 + siz], nodes2[0]->back(), nodes1[0]->back());		//extension arc for node state = |S2| + 1
			nodes2[0]->back()->gap_parent = nodes1[0]->back()->gap_child;
			nodes2[0]->back()->lng_arc = nodes1[0]->back()->gap_child;
			active = 1;
		}
		for (size_t j = 0; j < siz; ++j) {														//alignment arcs for node state = |S2| + 1
			if (nodes1[0]->back()->max_to[0] + value_ref[pos][j] + ext_gap_ref[lvl][siz - j - 1] + ext_gap_ref[0][j] + 0.0001 >= bound) {
				nodes2[0]->at(j)->max_to[0] = nodes1[0]->back()->max_to[0] + value_ref[pos][j] + ext_gap_ref[lvl][siz - j - 1] + ext_gap_ref[0][j];
				nodes1[0]->back()->children.emplace_back(new Arc(value_ref[pos][j] + ext_gap_ref[lvl][siz - j - 1] + ext_gap_ref[0][j], nodes2[0]->at(j), nodes1[0]->back()));
				nodes2[0]->at(j)->parents.emplace_back(nodes1[0]->back()->children.back());
				nodes2[0]->at(j)->lng_arc = nodes1[0]->back()->children.back();
				active = 1;
			}
	...
	for (int k = lvl - 1; k >= 0; --k) {							//defining alignment arcs for nodes state > 2
		for (int i = siz - 1; i >= 0; --i) {
			if (nodes1[k]->at(i)->gap_parent == NULL && nodes1[k]->at(i)->parents.empty()) {
				...
				if (nodes1[k]->at(i)->max_to[0] + ext_gap_ref[k][i + 1] + 0.0001 >= bound) {
					nodes2[k + 1]->at(i)->max_to[0] = nodes1[k]->at(i)->max_to[0] + ext_gap_ref[k][i + 1];
					nodes1[k]->at(i)->gap_child = new Arc(ext_gap_ref[k][i + 1], nodes2[k + 1]->at(i), nodes1[k]->at(i));		//defining gap arc for nodes state > 2
					nodes2[k + 1]->at(i)->gap_parent = nodes1[k]->at(i)->gap_child;
					nodes2[k + 1]->at(i)->lng_arc = nodes1[k]->at(i)->gap_child;
					active = 1;
				}
	...
	for (int k = nodes2.size() - 1; k >= 0; --k) {							//deleting last layer nodes with no parents
		for (int i = nodes2[k]->size() - 1; i >= 0; --i) {
			if (!nodes2[k]->at(i)->gap_parent && nodes2[k]->at(i)->parents.empty()) {
				nodes2[k]->at(i)->active = 0;
				//delete nodes2[k]->at(i);
				//nodes2[k]->erase(nodes2[k]->begin() + i);
			}
		}
		/*if (nodes2[k]->empty()) {
		delete nodes2[k];
		nodes2.erase(nodes2.begin() + k);
		}
		else
		nodes2[k]->shrink_to_fit();*/
	}
